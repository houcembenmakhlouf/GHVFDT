...
from skmultiflow.trees.split_criterion import GaussianHellingerDistanceCriterion
...
from skmultiflow.trees.nodes import GhActiveLearningNode
...
from skmultiflow.trees.nodes import GhLearningNodeNB
# from skmultiflow.trees.nodes import LearningNodeNBAdaptive
from skmultiflow.trees.nodes import GhLearningNodeNBAdaptive
...
class GhvfdtClassifier(BaseSKMObject, ClassifierMixin):
    ...
    def split_criterion(self, split_criterion):
        if split_criterion != self._GAUSSIAN_HELLINGER:
            print("Invalid split_criterion option {}', will use default '{}'".
                  format(split_criterion, self._GAUSSIAN_HELLINGER))
            self._split_criterion = self._GAUSSIAN_HELLINGER
        else:
    ...
    def _partial_fit(self, X, y, sample_weight):
        ...
        if isinstance(leaf_node, LearningNode):
            ...
            if self._growth_allowed and isinstance(learning_node, GhActiveLearningNode):
    ...
    def _new_learning_node(self, initial_class_observations=None, is_active_node=True):
        ...
        if is_active_node:
            if self._leaf_prediction == self._MAJORITY_CLASS:
                return GhActiveLearningNode(initial_class_observations)
            elif self._leaf_prediction == self._NAIVE_BAYES:
                return GhLearningNodeNB(initial_class_observations)
            else:  # NAIVE BAYES ADAPTIVE (default)
                return GhLearningNodeNBAdaptive(initial_class_observations)
        else:
    ...
    def _attempt_to_split(self, node: GhActiveLearningNode, parent: SplitNode, parent_idx: int):
    ...
    def enforce_tracker_limit(self):
        ...
        for i in range(cutoff):
            if isinstance(learning_nodes[i].node, GhActiveLearningNode):
    ...
    def estimate_model_byte_size(self):
        ...
        for found_node in learning_nodes:
            if isinstance(found_node.node, GhActiveLearningNode):
    ...
    def deactivate_all_leaves(self):
        ...
        for i in range(len(learning_nodes)):
            if isinstance(learning_nodes[i], GhActiveLearningNode):
                self._deactivate_learning_node(learning_nodes[i].node,
                                               learning_nodes[i].parent,
                                               learning_nodes[i].parent_branch)

    def _deactivate_learning_node(self, to_deactivate: GhActiveLearningNode, parent: SplitNode, parent_branch: int):
