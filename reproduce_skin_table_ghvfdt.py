...
from dataset.skin_data.split_skin import create_all_test_files
...
# test the tree on +1:-10 data:
    test_path = Path("dataset") / "skin_data" / str("test_1_"+str(imbalancy)+"_0.1.csv")
    fscore, gm = test_tree(test_path, trees_copies[0])
    result.append((fscore, gm))
    print("\n")

# test the tree on +1:-100 data:
    test_path = Path("dataset") / "skin_data" / str("test_1_"+str(imbalancy)+"_0.5.csv")
    fscore, gm = test_tree(test_path, trees_copies[1])
    result.append((fscore, gm))
    print("\n")

# test the tree on +1:-1000 data:
    test_path = Path("dataset") / "skin_data" / str("test_1_"+str(imbalancy)+"_0.75.csv")
    fscore, gm = test_tree(test_path, trees_copies[2])
    result.append((fscore, gm))
    print("\n")

# test the tree on +1:-10000 data:
    test_path = Path("dataset") / "skin_data" / str("test_1_"+str(imbalancy)+"_1.csv")
...
# pretrain on 200 pos and 1000 neg:
    train_path = Path("dataset") / "skin_data" / "train_200_1000.csv"
...
# save results:  
    ...
    result_file_path = Path("results") / "GHVFDT" / str("results_"+str(test_idx)+".csv")
...
if __name__ == "__main__":
    ...
    result_file_path = Path("results") / "skin" / "GHVFDT" / str("results_average.csv")
    with open(result_file_path, "w") as file:
        file.write(new_file_content) 
