...
def convert_data_to_csv():
    ...
    with open("Skin_NonSkin.csv", "w") as file:
...
def extract_pos_neg_data(random_idx):
    ...
    dataset_path = current_file_path / "Skin_NonSkin.csv"
...
def create_sub_set(file_path, pos_data, neg_data, n_pos, n_neg, random_idx):
    ...
    new_file_content = "B,G,R,class\n"
          
    with open(file_path, "w") as file:
        for _, anno in enumerate(data):
            new_line = str(anno[0])+','+str(anno[1])+','+str(anno[2])+','+str(anno[3])
            new_file_content += new_line + "\n"
